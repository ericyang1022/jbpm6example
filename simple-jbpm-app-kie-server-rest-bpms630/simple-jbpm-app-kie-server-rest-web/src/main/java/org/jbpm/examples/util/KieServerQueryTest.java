...
public class KieServerQueryTest {
...
//				.expression("select id, employee_no, employee_name, charge_item_name from timesheet")
				.expression("select distinct * from ProcessInstanceLog")
				.source("java:jboss/datasources/jbpmDS")
				.target("CUSTOM").build();
...
//			System.out.println("row: " + l);
		...
		// read the result
/*		for (timesheet ts : query) {
			System.out.println(ts);
		}*/
		
/*		List<ArrayList> query = KieServerUtils.getQueryServicesClient().query(QUERY_NAME,
		QueryServicesClient.QUERY_MAP_RAW, 0, 100, ArrayList.class);
